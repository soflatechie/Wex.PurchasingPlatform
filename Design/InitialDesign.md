...
- **Integration**: POST a transaction → 201 with id; GET the same transaction in a supported currency → correct converted amount and echoed rate; GET in an unsupported/unknown currency → 4xx; GET a nonexistent id → 404; PUT an existing id with valid/invalid fields → 200/400; PUT a nonexistent id → 404; DELETE an existing id then GET it → 404; a conversion request while `IExchangeRateProvider` is unreachable → `502`, while `PurchaseTransaction` CRUD (Requirement #1) keeps working regardless.
- **Contract/fake**: `TreasuryExchangeRateClient` tested against a fixture of a real single-row filtered Treasury API response (the actual shape of `GetLatestRateOnOrBeforeAsync`'s call) so a schema change in the live API is caught without depending on network access in CI. Rate-selection logic itself (`SelectRate`) is tested against a mocked `IExchangeRateProvider`, never a live or locally-cached dataset.
