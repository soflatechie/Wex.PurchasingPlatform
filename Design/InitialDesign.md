...
One capability is added beyond what was strictly asked for, as a deliberate choice to round out the exercise rather than a misreading of scope (see §2.3 for its design):
...
- `Id`: server-generated `Guid` (v7 if available for index-friendliness), returned to the caller on create — this is the unique identifier used in every API route/DTO.
- `TransactionNumber`: a plain autoincrementing integer, database-generated, returned alongside `Id`. A GUID satisfies "must uniquely identify the purchase" on its own, but isn't something a person can read back comfortably in a UI; `TransactionNumber` gives users a short, sequential number to reference (e.g. "PT-000042" as displayed in the desktop app) without changing what `Id` means anywhere else.
...
- **Genuine failure case**: if no published rate exists on or before the transaction date at all for the requested currency (e.g., the transaction predates the first time Treasury ever reported that currency, or the currency code doesn't appear in the dataset), conversion is impossible outright — the API returns `404`/`422` with a clear message. This is a distinct case from staleness: staleness means "we have an answer, flagged"; this means "we have no answer." A third case — Treasury being unreachable at request time — is distinct again and returns `502` (see §3's "Exchange rate source" row for why this trade-off is accepted).
- Conversion math: `converted = round(purchaseAmount * exchangeRate, 2, AwayFromZero)`. Both operands are `decimal`; the exchange rate itself is transmitted at full published precision, only the final converted total is rounded.
- **Where the rate comes from**: fetched live from Treasury at request time — one HTTP call, filtered server-side to `country_currency_desc` + `record_date <= transactionDate`, sorted descending, limited to one row — not read from a locally synced copy of the dataset. See §3 for why.
...
This filtering happens in two steps to keep it cheap (see §3's "Exchange rate source" row): a small local cache holds every `(country, currency)` identity Treasury has ever published, so picking a country cheaply lists its candidate currencies (usually 1–3) without touching Treasury; then each candidate is checked live against Treasury for a usable rate as of the transaction's date. The candidate count per country stays small regardless of how many currencies exist dataset-wide.
...
| Exchange rate source | HTTP client (`IHttpClientFactory`) against the Treasury Fiscal Data API, wrapped behind an `IExchangeRateProvider` interface, **queried live per request** — no local cache of rate values | A conversion only ever needs one specific row (one country/currency, on or before one date); the full dataset is ~15–20K rows going back to 2001, and pulling/maintaining all of it to answer a single-row question is pure overhead with no upside — Treasury also revises data with no change notification, so a locally cached copy can silently drift with no way to detect it. Only the small, essentially-static set of `(country, currency)` *identities* is cached locally (a one-time crawl, not a per-startup sync), so the currency picker doesn't need to live-query Treasury for every currency it's ever published just to build a dropdown. Tests use a fake/mocked `IExchangeRateProvider`, so this stays deterministic without live network calls despite not caching rate values. |
...
| Testing | xUnit with plain `Assert`, unit tests only (no FluentAssertions — its v8+ license requires a paid tier for larger organizations). Every dependency is mocked with Moq — repositories, `IExchangeRateProvider`, service interfaces — so no test ever touches a real database or makes a real HTTP call; there is no `WebApplicationFactory`/TestServer anywhere in the suite. Controllers are tested directly against mocked service interfaces, not through a running host. | Rounding and rate-selection edge cases are exactly the kind of logic that silently rots without tests; the take-home explicitly asks for "the automated tests you consider essential." Keeping every test fully mocked keeps the suite fast and deterministic with zero external dependencies. |
...
├── src/
│   ├── Wex.PurchasingPlatform.Api        (the whole server side — controllers, services, repositories, entities, EF Core, Treasury client — organized as folders within one project; see TechnicalDesign.md §2)
...
└── tests/
    └── Wex.PurchasingPlatform.Tests      (all tests, one project — unit tests only, every dependency mocked)
```
...
- **Unit — controllers**: each controller action tested directly against its injected service interfaces mocked with Moq (success → the expected `ActionResult` and payload; not-found → `404`; etc.) — no test host, no real HTTP pipeline, no real database. This project uses unit tests only; there is no integration-test category and no `WebApplicationFactory`/TestServer anywhere in the suite.
- **Contract/fake**: `TreasuryExchangeRateClient` tested against a fixture of a real single-row filtered Treasury API response (the actual shape of `GetLatestRateOnOrBeforeAsync`'s call) via a fake `HttpMessageHandler`, so a schema change in the live API is caught without ever making a network call. Rate-selection logic itself (`SelectRate`) is tested against a mocked `IExchangeRateProvider`, never a live or locally-cached dataset.
...
- **Exchange rate acquisition** — queried live from Treasury per request rather than proactively synced into a local copy of the ~15–20K-row dataset (see §3's "Exchange rate source" row): a conversion only ever needs one row, and a cached copy has no way to detect if Treasury has revised the source data since it was pulled. Only the small set of `(country, currency)` identities is cached locally, to keep the currency picker cheap.
...
- Update/delete added to scope, beyond what the client's requirements strictly asked for.
- Front end changed from a CLI to a Windows Forms desktop app; Angular replaced with Blazor.
- Exchange rate acquisition changed from a proactively-synced local copy of the full ~15–20K-row Treasury dataset to a live per-request lookup, with only currency *identities* (not rates) cached locally — see `TechnicalDesign.md`'s changelog for the full rationale.
- Added `TransactionNumber`, a sequential integer, alongside `Id` — a raw GUID isn't something worth showing a user in the desktop/web UI. `Id` still satisfies "must uniquely identify the purchase" and remains the identifier used in the API; `TransactionNumber` is purely a friendlier display value.
